...
import { defaultExerciseFormValues } from '@/helpers/ExercisesHelper';
import { useToast } from '@/hooks/use-toast';
import { uploadToImgBB } from '@/lib/imgBB';
import { cn } from '@/lib/utils';
import { IExerciseFormData, exerciseFormSchema } from '@/models/Schemas/ExerciseFormSchema';
import { IExercise, IExerciseBodyPart, IExerciseEquipment } from '@/models/Stores/Exercises/IExercise';
import { useExercisesStore } from '@/stores/exercisesStore';

export const CreateExerciseForm = ({ onFormClose }: { onFormClose: () => void }) => {
  const addExercise = useExercisesStore((store) => store.addExercise);
  const form = useForm({
  ...
  const onSubmit = async (data: IExerciseFormData) => {
    setIsSubmitting(true);
    try {
      const photoUrl = await uploadToImgBB(data.photo);
      const gifUrl = await uploadToImgBB(data.gif);
    ...
    <Form {...form}>
      <motion.div initial="hidden" animate="visible" variants={formAnimation} className="space-y-6">
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 px-1">
          ...
          <motion.div className="p-4 px-2 bg-muted rounded-lg shadow-md" variants={itemAnimation}>
          ...
          <motion.div variants={itemAnimation} className="pt-4">
            ...
            >
              <span className="relative z-10">{isSubmitting ? 'Создание...' : 'Создать упражнение'}</span>
            </Button>
          </motion.div>
        </form>
      </motion.div>
